...
{
	public $buildString = '<div class="row blck{$alt? alt}{$classes? $classes}" id="{$id}"><label for="{$id}_0">{$desc}{$required? <span class="required">*</span>}</label><div class="row"><input type="password" name="{$name}[0]" id="{$id}_0"{$validation? data-fio-validation="$validation"} /><input type="password" name="{$name}[1]" id="{$id}_1" /> (verify)</div>{$error?<p class="err">$error</p>}<p class="hint">{$hint}</p></div>';
