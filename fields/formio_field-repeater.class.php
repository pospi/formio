...
{
	public $buildString = '<div class="row repeater blck{$alt? alt}{$classes? $classes}" id="{$id}" data-name-base="{$name}"{$dependencies? data-fio-depends="$dependencies"}{$validation? data-fio-validation="$validation"} data-fio-type="repeater"{$sortable? data-fio-sortable="1"}>
