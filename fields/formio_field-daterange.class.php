...
class FormIOField_Daterange extends FormIOField_Date
{
	public $buildString = '<div class="row daterange{$alt? alt}{$classes? $classes}" id="{$id}"{$dependencies? data-fio-depends="$dependencies"}{$validation? data-fio-validation="$validation"}>
		...
		<input type="text" name="{$name}[0]" id="{$id}_start"{$value? value="$value"}{$readonly? readonly="readonly"} data-fio-type="date"{$required? data-fio-validation="requiredValidator"} />
		 - <input type="text" name="{$name}[1]" id="{$id}_end" value="{$valueEnd}"{$readonly? readonly="readonly"} data-fio-type="date"{$required? data-fio-validation="requiredValidator"} />
