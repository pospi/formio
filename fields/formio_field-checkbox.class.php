...
{
	public $buildString = '<div class="row checkbox{$alt? alt}{$classes? $classes}"><label class="checkbox"><input type="checkbox" name="{$name}" id="{$id}"{$disabled? disabled="disabled"}{$checked? checked="checked"}{$dependencies? data-fio-depends="$dependencies"} />{$desc}{$required? <span class="required">*</span>}</label>{$error?<p class="err">$error</p>}<p class="hint">{$hint}</p></div>';
